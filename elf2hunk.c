...
int sym_dump(int hunk_fd, struct elfheader* eh, struct sheader *sh, struct hunkheader **hh, int shid, int symtabndx, int flags)
{
	...
	for(i = 0; i < syms; i++) {
		...
		if(name[0] == '\0')
			continue;
		char name2[512];
		char* demangled = cplus_demangle_v3(name, 0);
		snprintf(name2, sizeof(name2), "%.96s@%x", demangled ? demangled : name, s.size); // there seems to be a limit of symbol names in AmigaDOS, 128 won't work.
		free(demangled);
		if(flags & F_VERBOSE)
			printf("\t0x%08x: %s\n", (int)s.value, name2);
